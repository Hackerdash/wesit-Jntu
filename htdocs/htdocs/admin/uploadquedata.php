...
$cname=$_SESSION["uploadexamcat"];
 if(isset($_POST['submit']))
 {
  ...
  for($i=1;$i<=$rowCount;$i++)
  {
    ...
    $isim=$data->val($i,8);
    $pm=$data->val($i,9);
    $nm=0;//default value ..if exam need negative marks replace 0 with $data->val($i,10)
    ...
    $sql = "insert into exam_quescache values ('$examid','$cname',$qno,'$ques','$o1','$o2','$o3','$o4','$to','$isim',$pm,$nm)";
    mysql_query($sql);
    echo "<tr style=background-color:#ccc;><td>".$qno."</td><td colspan=3>".$ques."</td></tr><tr><td colspan=3>";
    if($isim=='Y'||$isim=='y')
    {
    echo "<img src=../ques_images/$qno.jpg alt=image not uploaded properly width=300 height=300/>";
    }
    else
     {
     echo "No image found for the question";
     }
    echo "</td></tr><tr style=background-color:#FFBBFF><td>".$o1."</td><td>".$o2."</td><td>".$o3."</td><td>".$o4."</td></tr><tr><td colspan=2>".$to."</td><td>".$pm."</td><td>".$nm."</td></tr>";
